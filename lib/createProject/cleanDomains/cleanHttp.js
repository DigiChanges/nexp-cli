...
const removeAllExtraFiles = async(vars, httpMapper) =>
{
    ...
    {
        const ormFiles = await glob(resolve(`${vars.projectName}/src/**/*${http}*.ts`));
...
const cleanFilesInfo = async(vars, httpMapper) =>
{
    ...
    const httpChoosen = last(mapperFilter);

    const appFactoryFile = `import IApp from '../Application/Http/IApp';
import App${httpChoosen} from '../Application/Http/App${httpChoosen}';

class AppFactory
{
    static create(appName = 'App${httpChoosen}'): IApp
    {
        const strategy: Record<string, any> = {
            [App${httpChoosen}.name]: App${httpChoosen}
        };
...
export default AppFactory;
    ...
    await writeFile(resolve(`${vars.projectName}/src/Shared/Factories/AppFactory.ts`), appFactoryFile, { flag: 'w' });

		if (vars.orm !== 'MikroORM')
		{
	const options = {
		files: resolve(`${vars.projectName}/src/Shared/Application/Http/App${vars.http}.ts`),
